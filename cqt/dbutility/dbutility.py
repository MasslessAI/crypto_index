...
def getUrlFromCfg(db_cfg_file,db_id):
    ...
    url = '%s://%s:%s@%s:%s/%s' %(cfg_data['db_engine'],cfg_data['username'],cfg_data['password'],cfg_data['hostname'],cfg_data['port'],cfg_data['db_name'])
    return url
    
    
def genAddPrimaryKeySQL(tbl,key_field):
    add_primary_key = 'alter table "%s" add primary key (%s)' % (tbl,key_field)
...
def get_from_db(tbl_name,from_date='',to_date='',db_id = 'Amazon_RDS'):
    ...
    if from_date == '' and to_date == '': ## select all
        ...
        select_str = select([table]).where(table.c.key <= to_date)
    elif to_date == '':
        select_str = select([table]).where(table.c.key >= from_date)
    else:
