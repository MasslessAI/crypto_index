...
def data_gen(source, request, keyList=None, write_to_file=False, sync_to_db=True, returnDF=False):
    ...
    if source in api_cfg.keys():
        ...
        if not keyList is None:
            for key in keyList:
                ...
                if response.status_code == 200:
                    ...
                    print("Response yields a status code of %i (non-200), trying for next key" %
                          response.status_code)
        if response.status_code != 200:
            print(response.text)
            error('response yields %i status code, exiting...' %
                  response.status_code)
        ...
        if write_to_file:
            data_path = '../data/csv/'
            data_file_csv = source + '_' + \
                get_req_str(source, request, False) + '_data.csv'
            df.to_csv(data_path + data_file_csv, sep=',', encoding='utf-8')

            data_path = '../data/pickle/'
            data_file_pickle = source + '_' + \
                get_req_str(source, request, False) + '_data.pickle'
            ...
            for attr in api_cfg[source]['data'][request_type]:
                if attr[0] in request:
                    if "time_" in attr[0] and unix_time_flag == 'true':
                        t = datetime.strptime(request.get(
                            attr[0]), "%Y-%m-%dT%H:%M:%S")
                        t_unix = str(
                            int(mktime(t.timetuple()) + 1e-6 * t.microsecond))
                        request_str = request_str + attr[1] + t_unix
                    else:
                        request_str = request_str + \
                            attr[1] + request.get(attr[0])
        else:
